...
void DX11Renderer::setMultiSamplingCount(uint32_t count)
{
	...
	UINT qualityLevels;
	CHECK_HRESULT(
		m_device->CheckMultisampleQualityLevels(swDesc.BufferDesc.Format, count, &qualityLevels));

	if(qualityLevels == 0)
	{
		Common::error("invalid argument: count (%u), sampling count is not supported, see "
					  "GetSupportedMultiSamplingCounts()",
			count);
...
SwapChainDisplayMode DX11Renderer::displayMode() const
{
	...
	uint32_t refreshRate = (uint32_t)(desc.BufferDesc.RefreshRate.Numerator /
										  (float)desc.BufferDesc.RefreshRate.Denominator +
									  0.5f);
...
std::vector<OutputDisplay> DX11Renderer::supportedOutputs() const
{
	...
	while(m_adapter->EnumOutputs((UINT)index, mst::initialize(o)) != DXGI_ERROR_NOT_FOUND)
	{
		...
		auto charCount = WideCharToMultiByte(
			CP_UTF8, 0, od.DeviceName, (int)wcslen(od.DeviceName), nullptr, 0, nullptr, nullptr);
		...
		if(charCount > 0)
		{
			...
			WideCharToMultiByte(CP_UTF8, 0, od.DeviceName, (int)wcslen(od.DeviceName),
				result.data(), charCount, nullptr, nullptr);
		}
...
std::vector<SwapChainDisplayMode> DX11Renderer::supportedDisplayModes(
	uint32_t outputIndex, PixelFormat format) const
{
	...
	for(auto& desc : descs)
	{
		uint32_t refreshRate =
			(uint32_t)(desc.RefreshRate.Numerator / (float)desc.RefreshRate.Denominator + 0.5f);
