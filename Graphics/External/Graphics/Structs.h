...
struct Viewport
{
	...
	Viewport(const float2& xy, const float2& size)
		: x(xy.x)
		, y(xy.y)
		, width(size.x)
		, height(size.y)
...
struct ShaderResourceInfo
{
	...
	explicit ShaderResourceInfo(
		uint64_t nameHash, ResourceType type, uint32_t slot, uint32_t space = 0)
		: nameHash(nameHash)
		, type(type)
		, slot(slot)
		, space(space)
...
struct ShaderBufferInfo : ShaderResourceInfo
{
	...
	explicit ShaderBufferInfo(uint64_t nameHash, ResourceType type, uint32_t slot,
		size_t dataSize = 0, uint32_t space = 0)
...
struct ShaderTextureInfo : ShaderResourceInfo
{
	...
	explicit ShaderTextureInfo(uint64_t nameHash, ResourceType type, uint32_t slot,
		bool isArray = false, uint32_t space = 0)
		: ShaderResourceInfo(nameHash, type, slot, space)
	{ }
...
{
	SwapChainDisplayMode(uint32_t width, uint32_t height, uint32_t refreshRate)
...
{
	SwapChainCreationSettings(PixelFormat pixelFormat, uint32_t refreshRate, bool vSync,
		uint32_t multiSamplingCount = 1, uint32_t preferredOutput = 0)
...
{
	OutputDisplay(uint32_t index, std::string name)
		: index(index)
		, name(name)
	{ }
...
{
	FullScreenState(bool isFullScreen, uint32_t outputIndex)
