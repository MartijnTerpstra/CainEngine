...
#define GRAPHICS_INIT_INST_FUNCTION(x)                                                             \
	x = (PFN_##x)vkGetInstanceProcAddr(Instance, #x);                                              \
	if(x == nullptr)                                                                               \
	Common::fatalError("APIFunctions::Init(): extensions function not loaded: " #x)

#define GRAPHICS_INIT_DEV_FUNCTION(x)                                                              \
	x = (PFN_##x)vkGetDeviceProcAddr(Device, #x);                                                  \
	if(x == nullptr)                                                                               \
	Common::fatalError("APIFunctions::Init(): extensions function not loaded: " #x)
	...
	size_t, int32_t msgCode, const char* pLayerPrefix, const char* pMsg, void*)
{
	...
	if(msgFlags & VK_DEBUG_REPORT_ERROR_BIT_EXT)
	{
		...
		CainEngine::Common::error(
			"Vulkan API ERROR: [%s] Code %d : %s", pLayerPrefix, msgCode, pMsg);
	...
	else if(msgFlags & VK_DEBUG_REPORT_WARNING_BIT_EXT)
	{
		if(strstr(pMsg, "vkGetFenceStatus called for fence") ||
			...
			strstr(pMsg, "prior to any Draw Cmds. It is recommended you use RenderPass "
						 "LOAD_OP_CLEAR on Attachments prior to any Draw."))
			return false;
		CainEngine::Common::warning(
			"Vulkan API WARNING: [%s] Code %d : %s", pLayerPrefix, msgCode, pMsg);
...
APIFunctions::APIFunctions(
	const std::shared_ptr<VkInstance_T>& instance, VkDevice device, bool debuglayer)
	...
	, m_instanceHandle(instance)
{
	if(debuglayer)
	{
		...
		checkVkResult(
			vkCreateDebugReportCallbackEXT(Instance, &debugInfo, nullptr, &m_debugCallback),
			"Graphics::Internal::VulkanObjectFactory::VulkanObjectFactory(): call failed: "
